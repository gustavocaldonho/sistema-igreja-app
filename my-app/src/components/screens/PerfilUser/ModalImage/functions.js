...
export const getImageProfile = async (cpf, setImage, setLoadingImage) => {
  try {
    const token = await AsyncStorage.getItem("AccessToken");
    const result = await getImageUser(token, cpf);
...
export const handleImagePicker = async (cpf, setLoadingImage, setImage) => {
  ...
  if (!granted) {
    ...
    if (!canceled) {
      ...
      try {
        ...
        const token = await AsyncStorage.getItem("AccessToken");
        const result = await setImageUser(formData, token, cpf);
        ...
        if (result.status !== 204) {
          ...
          getImageProfile(cpf, setImage, setLoadingImage); // Atualizar a imagem após o envio
        }
      } catch (error) {
        AlertMsg(error);
      }
...
export const deleteImageProfile = async (cpf) => {
  try {
    const token = await AsyncStorage.getItem("AccessToken");
    const result = await deleteImageUser(token, cpf);
    if (result.status !== 204) {
